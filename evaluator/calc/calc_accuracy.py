...
from pipeline.lib.import_openai_result import (
    import_openai_prediction_result,
    import_openai_prediction_result_impl,
)
from pipeline.lib.import_zerogpt_result import (
    import_zerogpt_prediction_result,
    import_zerogpt_prediction_result_impl,
)
...
def calc_t5_specific_class_hidden(prediction_idx: int, positive_class: str):
    ...
    return quick_statistics_binary(
        reformulated_predictions, positive_class, threshold=0.5
    )
...
def calc_t5_full_statistics(digit=3):
    ...
    for cate_idx, category in enumerate(categories):
        ...
        print(
            f"TPR: {round(tpr(*stat), digit)},\tTNR: {round(tnr(*stat), digit)}, "
            f"FPR: {round(fpr(*stat), digit)},\tFNR: {round(fnr(*stat), digit)} "
        )
        print(f"Acc: {round(acc(*stat), digit)}")
        print(f"F1 : {round(f1(*stat), digit)}")
        print(
            f"Recall: {round(recall(*stat), digit)},\tPrecision: {round(precision(*stat), digit)}"
        )
    ...
    print(
        f"Recall: {round(sum(recalls) / len(recalls), digit)},\tPrecision: {round(sum(precisions) / len(precisions), digit)}"
    )
...
def calc_openai_baseline_statistics(digit=3):
    ...
    stat = quick_statistics_binary(
        reformulated_predictions, pos_label="openweb", threshold=0.5
    )
...
def calc_zerogpt_baseline_statistics_detail(path: Path):
    def implementation(digit=3):
        ...
        prediction1 = import_zerogpt_prediction_result_impl(
            Path(
...
def calc_openai_baseline_statistics_detail(path: Path):
    def implementation(digit=3):
        ...
        prediction1 = import_openai_prediction_result_impl(
...
def calc_t5_full_statistics_detail(file_name: str, pos_label: str, pos_index: int):
    def implementation(digit=3):
        ...
        stat = quick_statistics_binary(
            reformulated_predictions, pos_label, threshold=0.5
        )
        ...
        with open("temp_result.txt", "a") as f:
            f.write(
                f" & {str(round(auc(curve[0], curve[1]), digit))[1:]} & {str(round(f1(*stat), digit))[1:]}"
            )

    return implementation


def calc_t5_hidden_statistics_detail_full(
    file_name: str, pos_label: str, pos_index: int
):
    def implementation(digit=3):
...
def calc_t5_hidden_statistics_detail(path: Path):
    def implementation(digit=3):
        prediction = T5_hidden.evaluate_predictions(
            [path, Path("data", "split", "Human", "test-dirty.jsonl")]
        )
        reformulated_predictions = []
        for entry in prediction:
            p_selected = entry["data"][0]
        ...
        stat = quick_statistics_binary(reformulated_predictions, "Human", threshold=0.5)
        curve = get_roc_binary(reformulated_predictions, pos_label="Human")
...
def calculate_punctuation_removal(
    filename: str, pos_label: str, pos_index: int, singleton: str
):
...
def calculate_punctuation_removal_binary(path: Path, singleton: str):
    def punctuation_removal(digit=3):
        ...
        stat = quick_statistics_binary(
            reformulated_predictions, "openweb", threshold=0.5
        )
        curve = get_roc_binary(reformulated_predictions, pos_label="openweb")
...
if __name__ == "__main__":
    TASKS = [
        ...
        ################################################################################################################
        # For table 5.
        # calc_openai_baseline_statistics_detail(
        ...
        #         "data",
        #         "baselines",
        #         "zerogpt_classifier_output",
        #         "open-llama-text.jsonl",
        #     )
        # ),
        # calc_zerogpt_baseline_statistics_detail(
