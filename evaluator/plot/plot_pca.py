...
def plot_t5_full_pca_compare():
    ...
    hiddens = T5_Full.evaluate_hidden_states(
        [
            Path("data", "split", "Human", "test.variant3.jsonl"),
            Path("data", "split", "GPT-4", "test.variant3.jsonl"),
            Path("data", "split", "Claude-Instant-v1", "test.variant3.jsonl"),
            Path("data", "split", "Gemini-Pro", "test.variant3.jsonl"),
        ]
    )
    ...
    for transformed, label in zip(Tarrays, labels):
        random_mask = np.random.randn(*(transformed[:, 0].shape)) < 0.2
        ax.scatter(
            transformed[random_mask, 0], transformed[random_mask, 1], label=label, s=1
        )
